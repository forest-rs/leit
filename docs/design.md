...
`leit_postings` provides sorted postings lists and cursor traits. The current
index execution path uses its own internal postings representation rather than
the `leit_postings` cursor traits; wiring the cursor-based traversal into the
index executor is Phase 2 work.
...
implementation, block pruning is only wired into the direct root-term fast
path. The general boolean evaluator materializes matches eagerly before
feeding them to the collector.
...
- wire `leit_postings` cursor traits into the index execution path, including
  `PostingsView`, `DecodeScratch`, and `MaxScoreScorer` from the handover spec
- expand the segment format beyond the current Phase 1 representation, adding
...
- introduce a `Projection<E>` trait as the application-facing API for
  projecting entities into retrieval fields, replacing the current raw
  field/value builder interface
- introduce `Retriever` and `FusionStrategy` traits for composable retrieval
  pipelines, generalizing the current concrete `RrfFusion` and direct index
  search methods
- consider migrating `QueryNode` children from `Vec<QueryNodeId>` to
  `Range<u32>` into a side table for cache efficiency once query evaluation
