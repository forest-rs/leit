...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Use the same analyzer configuration for indexing and querying so text is
    // normalized consistently on both paths.
    ...
    builder.index_document(
        ...
        &[
            ...
            (
                BODY,
                "ÜBER systems benefit from Unicode-aware normalization",
            ),
        ],
    )?;
    ...
    // The indexed body contains uppercase `ÜBER`, while the query uses
    // lowercase `über`.
    print_hits(
        "über",
        workspace.search(&index, "über", 5, SearchScorer::bm25())?,
    );
