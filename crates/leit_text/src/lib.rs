...
impl Tokenizer for WhitespaceTokenizer {
    fn tokenize<'a>(&self, text: &'a str, output: &mut Vec<Token<'a>>) {
        ...
        for (i, c) in text.char_indices() {
            if c.is_whitespace() {
                if in_token {
                    output.push(Token::new(&text[start..i], position, start, i));
                    position = position.checked_add(1).expect("token position overflow");
                    in_token = false;
                }
            } else if !in_token {
        ...
        if in_token {
            output.push(Token::new(&text[start..], position, start, text.len()));
        }
    }
}
...
fn apply_case_mapping(text: &str, case_mapping: CaseMapping) -> String {
    match case_mapping {
        ...
        CaseMapping::Fold => CaseMapper::new().fold_string(text),
    }
}
