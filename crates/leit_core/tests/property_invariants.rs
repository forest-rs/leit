...
proptest! {
    #[test]
    fn finite_scores_round_trip(value in -1_000_000.0f32..1_000_000.0f32) {
    ...
    fn bounded_score_arithmetic_stays_finite(
        a in -1_000_000.0f32..1_000_000.0f32,
        b in -1_000_000.0f32..1_000_000.0f32,
        weight in -100.0f32..100.0f32,
    ) {
        let lhs = Score::new(a);
        let rhs = Score::new(b);
    ...
    fn scored_hit_order_matches_score_then_id(
        ...
        right in -10_000.0f32..10_000.0f32,
        left_id in any::<u32>(),
        right_id in any::<u32>(),
    ) {
