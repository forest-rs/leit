...
#[test]
fn test_planner_produces_empty_plan_for_unknown_term() {
    let planner = Planner::new();
    let dictionary = TestDictionary;
    let fields = TestFieldRegistry;
    let mut scratch = PlannerScratch::new();
    ...
    let plan = planner
        ...
        .expect("unknown terms should produce an empty plan");

    // The plan should contain an empty AND node (matches nothing)
    assert_eq!(plan.program.node_count(), 1, "should have one empty node");
}
