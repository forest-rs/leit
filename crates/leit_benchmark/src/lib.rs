...
mod tests {
    ...
    fn benchmark_run_produces_stable_hit_shapes() {
        ...
        assert!(
            report.query_runs[0].hit_ids.contains(&1),
            "rust should match doc 1: {:?}",
            report.query_runs[0].hit_ids
        );
        assert!(
            report.query_runs[0].hit_ids.contains(&2),
            "rust should match doc 2: {:?}",
            report.query_runs[0].hit_ids
        );
        assert_eq!(report.query_runs[1].name, "retrieval");
        assert!(
            report.query_runs[1].hit_ids.contains(&1),
            "retrieval should match doc 1: {:?}",
            report.query_runs[1].hit_ids
        );
        assert!(
            report.query_runs[1].hit_ids.contains(&3),
            "retrieval should match doc 3: {:?}",
            report.query_runs[1].hit_ids
        );
