...
fn document_strategy() -> impl Strategy<Value = (Vec<usize>, Vec<usize>)> {
    vec(0_usize..TOKEN_POOL.len(), 1..6).prop_flat_map(|token_indexes| {
        let separator_count = token_indexes.len().saturating_sub(1);
        let separators = vec(0_usize..WHITESPACE_POOL.len(), separator_count);
...
proptest! {
    ...
    fn unicode_token_search_matches_expected_documents(
        ...
        query_index in 0_usize..TOKEN_POOL.len(),
    ) {
